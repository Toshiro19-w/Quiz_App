...
{
    public partial class MoMoSubscriptionPaymentForm : Form
    {
        private readonly SubscriptionPaymentService _paymentService;
        private readonly int _userId;
        private readonly int _durationMonths;
        private string _currentOrderId = "";

        public bool PaymentCompleted { get; private set; }

        /// <summary>
        /// Constructor cho thanh toán subscription
        /// </summary>
        /// <param name="userId">ID người dùng</param>
        /// <param name="durationMonths">Số tháng đăng ký (1, 6, hoặc 12)</param>
        public MoMoSubscriptionPaymentForm(int userId, int durationMonths)
        {
            InitializeComponent();
            _paymentService = new SubscriptionPaymentService();
            _userId = userId;
        ...
        private void InitializeComponent()
        {
            ...
            this.Name = "MoMoSubscriptionPaymentForm";
            this.ResumeLayout(false);
        }

        private void SetupForm()
        {
            var planName = _durationMonths switch
            {
                1 => "1 Tháng",
                6 => "6 Tháng",
                12 => "1 Năm",
                _ => $"{_durationMonths} Tháng"
            };

            this.Text = "Thanh toán Subscription - MoMo";
            ...
            {
                Text = $"Đăng ký gói Premium {planName}",
        ...
        private async Task StartPaymentAsync()
        {
            try
            {
                ...
                var result = await _paymentService.PaySubscriptionWithMoMoAsync(_userId, _durationMonths);
        ...
        private async Task StartPaymentStatusCheckAsync()
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                ...
                try
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"Subscription Poll {i + 1}: resultCode={resp?.resultCode}, status={resp?.status}");
                    ...
                    {
                        if (await _paymentService.CompleteSubscriptionPaymentAsync(_currentOrderId))
                        {
                            ...
                            statusLabel.ForeColor = Color.Green;
                            ToastHelper.Show(this, "Đăng ký Premium thành công!");
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Subscription Poll error: {ex.Message}");
                }
            }
